...
void Scenario_02_Alloc_Problems(resprov_loaded_ptrs& fpc)
{
    ...
    size_t rows = 105454554;
    size_t cols = 1050000050;

    for (size_t i = 0; i < MATMAX; ++i)
    {
        int8_t counter = fpc.SuperbMatrixCount();

        std::cout << TAG << " Going to create new (" << i << "/" << MATMAX
                  << ") SuperB Matrix!!! " << std::endl;

        mat_desc[i] = fpc.CreateSuperbMatrix(rows, cols);
        if (counter == fpc.SuperbMatrixCount())
        {
            std::cout << TAG << " FAILED to create new (" << i << "/" << MATMAX
                      << ") SuperB Matrix!!!11 " << std::endl;
            std::cout << TAG << " I'm done!" << std::endl;
            return;
        }
